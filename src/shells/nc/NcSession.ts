...
// ── Keywords para detectar misión de listener ─────────────────────
const LISTENER_KEYWORDS = ['listener', 'setup listener', 'escucha', 'nc -nlvp', 'netcat'];

function findListenerMissionId(ctx: ShellContext): number | undefined {
  if (!Array.isArray(ctx.allMachines)) return undefined;
  for (const machine of ctx.allMachines) {
    ...
    if (step && step.id === ctx.currentMissionId) return step.id;
  }
  return undefined;
}
...
Examples:
  ...
  nc -l -p 9999           Listen on port 9999 (without verbose)`,
    ...
    if (listenerResult.isListener) {
      ...
      const listenerMissionId = findListenerMissionId(ctx);
