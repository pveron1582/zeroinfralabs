...
export const scenario_01: Scenario = buildScenario({
  ...
  targetMachine: {
    ...
    files: [
      ...
      createFile('/var/www/html/index.php', `<?php
/**
 * ZI Blog — Front Page
 * WordPress 6.0 — Theme: zinfra-blog v1.2
 */
get_header();
?>
<main class="site-main">
  ...
  <article class="post">
    <h2 class="post-title">DeepSeek Flash: the new open model that rivals frontier AI</h2>
    <div class="post-date">Published on 2026-08-01</div>
    <p class="post-excerpt">DeepSeek has launched DeepSeek Flash, a new generation of artificial intelligence
    models that is already shaking up the industry. With impressive performance at a fraction of the cost of
    proprietary systems, DeepSeek Flash stands at the level of frontier models, offering advanced reasoning,
    code generation, and natural language understanding.</p>
  </article>

  <article class="post">
    <h2 class="post-title">Cybersecurity and AI: How artificial intelligence is transforming digital defense</h2>
    <div class="post-date">Published on 2026-07-31</div>
    <p class="post-excerpt">The integration of artificial intelligence in cybersecurity is marking a before and after
    in system protection. Companies are adopting AI-based solutions to detect threats in real time and analyze
    attack patterns, while attackers also leverage these technologies to create more sophisticated malware.</p>
  </article>

  <article class="post">
    <h2 class="post-title">New critical vulnerability reported in WordPress affects millions of sites</h2>
    <div class="post-date">Published on 2026-07-30</div>
    <p class="post-excerpt">Security researchers have discovered a remote code execution (RCE) vulnerability in
    WordPress that affects versions prior to 6.4.3. It is recommended to immediately update to the latest version
    and review server logs for suspicious activity.</p>
  </article>
</main>
<?php
get_footer();
      `.trim(), 'text', 'www-data', 'www-data', 0o644),
      createFile('/var/www/html/wp-config.php', `<?php
/** wp-config.php — ZI Blog (WordPress 6.0) */
...
define( 'AUTH_KEY',         'y0u-nev3r-gu3ss-th1s' );
define( 'SECURE_AUTH_KEY',  'd0nt-l34k-any0f-th1s' );
define( 'LOGGED_IN_KEY',    's3cr3t-s4lt-h3re' );
define( 'NONCE_KEY',        '4n0th3r-s4lt' );
define( 'AUTH_SALT',        'w4y-t00-m4ny-s4lts' );
define( 'SECURE_AUTH_SALT', '1-m34n-1t' );
define( 'LOGGED_IN_SALT',   'pl3ase-just-w0rk' );
define( 'NONCE_SALT',       'ok-f1ne' );
...
require_once ABSPATH . 'wp-settings.php';
      `.trim(), 'text', 'www-data', 'www-data', 0o640),
      createFile('/var/www/html/wp-content/.dir', '', 'text', 'www-data', 'www-data'),
      createFile('/var/www/html/wp-content/uploads/.dir', '', 'text', 'www-data', 'www-data'),
...
WP_ROOT = /var/www/html
    ...
    // Tabla de passwords del sistema: root + usuarios normales. `su` y el
    // login local validan contra estas contraseñas (ver utils/credentials.ts).
    known_passwords: {
      root: scenario01Data.credentials.ssh.pass,
      admin: scenario01Data.credentials.wpAdmin.pass,
      mario: 'password',
      sara: 'sunshine',
    },
  },
