...
describe('cmd_id', () => {
  it('debe mostrar uid/gid/grupos del usuario actual', () => {
    const machine = makeMachine({
      id: 'target-01',
      found_credentials: [{ file: '', user: 'user', pass: 'pass', verified: true, service: 'ssh' }],
    });
    const context = { machine } as CommandContext;
    const result = cmd_id.execute([], context);
    expect(result.output).toContain('uid=1000(user)');
    expect(result.output).toContain('gid=1000(user)');
    expect(result.isError).toBe(undefined);
  });

  it('debe mostrar info de otro usuario con argumento', () => {
    const machine = makeMachine();
    const context = { machine } as CommandContext;
    const result = cmd_id.execute(['developer'], context);
    expect(result.output).toContain('uid=1001(developer)');
    expect(result.output).toContain('gid=1001(developer)');
    expect(result.output).toContain('developers(2000)');
    expect(result.output).toContain('staff(2001)');
    expect(result.isError).toBe(undefined);
  });
  ...
  it('debe mostrar error si el usuario no existe', () => {
    const machine = makeMachine();
    const context = { machine } as CommandContext;
    const result = cmd_id.execute(['noexiste'], context);
