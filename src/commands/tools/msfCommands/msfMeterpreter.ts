// ── commands/tools/msfCommands/msfMeterpreter.ts ───────────────────
...
/**
 * Handle meterpreter session commands
 * Returns null if command not handled by meterpreter context
 */
export const executeMeterpreterCommand = (
  ...
  // Only handle if session is open
  if (!state.sessionOpen) return null;

  // exit in meterpreter closes the session, returns to exploit module prompt
