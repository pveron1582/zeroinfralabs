...
// Templates reutilizables para crear escenarios
// Evita duplicación de código en los archivos de ejercicios

import { assignDHCP } from '../utils/network';
import type { Machine, Scenario, MachineInfo, Port, LearningStep, Mission } from '../types';

// ── Template de máquina atacante (Kali Linux) ─────────────────────
const ATTACKER_MACS = [
  '08:00:27:AA:BB:CC',
  '08:00:27:AA:BB:CD',
  '08:00:27:AA:BB:CE',
...
// Reset counter para evitar reutilización de MACs entre escenarios
export function resetAttackerCounter() {
  attackerCount = 0;
}

export function createAttackerMachine(
  networkRange: string,
  ...
  return {
    id: 'attacker-01',
    machine_info: {
      hostname: customHostname || 'kali-attacker',
      ip: '', // Asignado por assignDHCP
      mac,
...
interface ScenarioBuilderConfig {
  ...
  networkRange: string;
  targetMachine: Omit<Machine, 'machine_info' | 'id'> & {
    id: string;
...
export function buildScenario(config: ScenarioBuilderConfig): Scenario {
  ...
  // Crear máquina objetivo con IPs asignadas dinámicamente
  const target: Machine = {
    ...config.targetMachine,
    machine_info: {
      ...config.targetMachine.machine_info,
      ip: '', // Asignado por assignDHCP
    } as MachineInfo,
    scan_results: { ports: config.targetMachine.ports },
    web_enumeration: config.targetMachine.web_enumeration || {
      web_server: 'none',
      cms: 'none',
      directories: [],
    },
    discovery_level: 0,
    learning_steps: config.learningSteps.map((step, idx) => ({
      ...step,
      id: idx + 1,
    })),
  ...
  // Asignar IPs dentro del rango de red del escenario
  const machines = assignDHCP(config.networkRange, [attacker, target]);

  // Generar misiones automáticamente de los learning steps
  const missions: Mission[] = config.learningSteps.map((step, idx) => ({
    id: idx + 1,
    title: step.task,
    description: step.text,
    status: idx === 0 ? 'active' : 'pending',
    targetMachineId: config.targetMachine.id,
    discoveryLevel: step.discoveryLevel,
  }));
...
// ── Templates predefinidos de puertos ─────────────────────────────
export const COMMON_PORTS = {
  ssh: (version = 'OpenSSH 8.2p1 Ubuntu', creds?: { user: string; pass: string }): Port => ({
    port: 22,
    protocol: 'tcp',
    state: 'open',
    service: 'ssh',
  ...
  http: (version = 'Apache httpd 2.4.41'): Port => ({
    port: 80,
    protocol: 'tcp',
    state: 'open',
    service: 'http',
  ...
  mysql: (state: 'open' | 'filtered' | 'closed' = 'filtered'): Port => ({
    port: 3306,
    protocol: 'tcp',
    state,
    service: 'mysql',
    version: state === 'open' ? 'MySQL 5.7.38' : 'unknown',
  }),
  smb: (version = 'Windows 7 Professional 7601'): Port => ({
    port: 445,
    protocol: 'tcp',
    state: 'open',
...
// ── Helper para crear directorios web ─────────────────────────────
export function createWebDirs(
  paths: Array<{ path: string; status: 200 | 301 | 403 | 404; description: string }>
) {
  return paths.map(p => ({
    path: p.path,
    status: p.status,
...
// ── Plantillas de escenarios comunes ───────────────────────────────
export const SCENARIO_TEMPLATES = {
  wordpress: (config: {
    ...
    targetMachine: {
      ...
      learning_steps: [],
      ports: [
        COMMON_PORTS.ssh('OpenSSH 8.2p1 Ubuntu', config.credentials),
        COMMON_PORTS.http('Apache httpd 2.4.41'),
        COMMON_PORTS.mysql('filtered'),
      ],
      web_enumeration: {
        web_server: 'Apache/2.4.41',
        cms: `WordPress ${config.wpVersion || '6.0'}`,
        directories: [
      ...
      files: [
        createFile('/home/admin/user.txt', config.flags.user),
        createFile('/root/flag.txt', config.flags.root),
        createFile('/uploads/config.bak', `DB_USER=${config.credentials.user}\nDB_PASS=${config.credentials.pass}`, 'text'),
      ],
    },
    learningSteps: [
      { id: 1, task: 'Reconocimiento de red', text: 'Descubrir osts activos: arp-scan <network/cidr>', targetMachineId: `lab-${config.id}-wp`, discoveryLevel: 1 },
      { id: 2, task: 'Escaneo de puertos', text: 'Escanear puertos: nmap -sV <target-ip>', targetMachineId: `lab-${config.id}-wp`, discoveryLevel: 2 },
      { id: 3, task: 'Enumeración Web', text: "Acceder al sitio web desde el botón de Firefox en la barra superior.", targetMachineId: `lab-${config.id}-wp`, discoveryLevel: 2 },
  ...
  lfiRce: (config: {
    ...
    id: config.id,
    name: config.name,
    description: 'Explota una vulnerabilidad de Local File Inclusion (LFI) para ejecutar una shell remota (RCE).',
    difficulty: 'Medium',
    category: 'Web',
    networkRange: config.networkRange,
    targetMachine: {
      id: `lab-${config.id}-lfi`,
      machine_info: {
      ...
      files: [
        ...
        createFile('/var/www/html/flag.txt', config.flags.root),
      ],
    },
    learningSteps: [
      ...
      { id: 3, task: 'LFI Discovery', text: 'Prueba leer archivos del sistema con ?page=../../../../etc/passwd', targetMachineId: `lab-${config.id}-lfi`, discoveryLevel: 3 },
      { id: 4, task: 'File Upload', text: 'Sube una reverse shell PHP en la sección de mantenimiento.', targetMachineId: `lab-${config.id}-lfi`, discoveryLevel: 3 },
      { id: 5, task: 'Remote Code Execution', text: 'Ejecuta tu shell usando LFI: ?page=uploads/tu_archivo.php', targetMachineId: `lab-${config.id}-lfi`, discoveryLevel: 4 },
    ],
  }),
  ...
  sshBrute: (config: {
    ...
    targetMachine: {
      ...
      learning_steps: [],
      ports: [
        COMMON_PORTS.ssh('OpenSSH 8.9p1 Ubuntu', config.credentials),
    ...
    learningSteps: [
      { id: 1, task: 'Reconocimiento de red', text: 'Descubrir hosts activos: arp-scan <network/cidr>', targetMachineId: `lab-${config.id}-ssh`, discoveryLevel: 1 },
      ...
      { id: 4, task: 'Acceso por SSH', text: 'Conectarse al servidor: ssh root@<target-ip> <password>', targetMachineId: `lab-${config.id}-ssh`, discoveryLevel: 4 },
    ],
  }),
  ...
  eternalBlue: (config: {
    ...
    id: config.id,
    name: config.name,
    description: 'Explotación de vulnerabilidad EternalBlue en Windows 7 sin parchear mediante Metasploit.',
    difficulty: 'Medium',
    category: 'Network',
    networkRange: config.networkRange,
    targetMachine: {
      ...
      web_enumeration: { web_server: 'none', cms: 'none', directories: [] },
      files: [
        createFile('C:\\\\Users\\\\Administrator\\\\Desktop\\\\flag.txt', config.flags.root),
        createFile('C:\\\\Windows\\\\System32\\\\config\\\\SAM', '[SAM Database — use hashdump]', 'binary'),
      ],
    },
    learningSteps: [
      { id: 1, task: 'Reconocimiento de red', text: 'Descubrir hosts activos: arp-scan <network/cidr>', targetMachineId: 'win7-target', discoveryLevel: 1 },
      { id: 2, task: 'Escaneo de puertos', text: 'Identificar servicios: nmap -sV <target-ip>', targetMachineId: 'win7-target', discoveryLevel: 2 },
      { id: 3, task: 'Verificar vulnerabilidad', text: 'En msfconsole: use auxiliary/scanner/smb/smb_ms17_010 → set RHOSTS <target-ip> → run', targetMachineId: 'win7-target', discoveryLevel: 2 },
      ...
      { id: 5, task: 'Verificar acceso SYSTEM', text: 'En el meterpreter: getuid', targetMachineId: 'win7-target', discoveryLevel: 4 },
    ],
  }),
};
