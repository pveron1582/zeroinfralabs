...
function ScenarioCard({ scenario, index, onSelect }: { scenario: Scenario; index: number; onSelect: (id: string) => void; }) {
  ...
  return (
    <article onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      className="flex flex-col overflow-hidden select-none w-full"
      ...
      <div className="p-4 flex flex-col" style={{ minHeight: '220px' }}>
        ...
        <div className="mt-auto pt-3 flex items-center justify-between" style={{ borderTop: '1px solid #1e2d2d' }}>
...
export function LabGrid() {
  ...
  const handleSelect = (id: string) => {
    // Always use URL lang param to ensure URL and store stay in sync
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#0b1015', fontFamily: "'Cascadia Code','Fira Code','Consolas',monospace" }}>
      ...
      <header className="relative z-10 flex items-center justify-between px-6 md:px-8 py-4" style={{ borderBottom: '1px solid #1c2a2a' }}>
        <Link to={`/${language}`}
          className="flex items-center gap-2 text-sm text-gray-400 hover:text-emerald-400 transition-colors font-mono">
      ...
      {/* Title */}
      <section className="relative z-10 px-6 md:px-8 pt-12 pb-6 text-center">
        <h1 className="text-2xl md:text-3xl font-bold mb-2" style={{ color: '#d1d5db' }}>
          <span style={{ background: 'linear-gradient(100deg, #10b981 0%, #22d3ee 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{t('labsPageTitle')}</span>
        </h1>
        <p className="text-sm text-gray-500 max-w-md mx-auto">{t('labsPageSubtitle')}</p>
      </section>

      {/* Labs grid */}
      <main className="relative z-10 flex-1 px-4 md:px-6 pb-14">
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 justify-items-center max-w-[1600px] mx-auto">
          {SCENARIOS.map((s, i) => (<ScenarioCard key={s.id} scenario={s} index={i} onSelect={handleSelect} />))}
        </div>
      </main>
