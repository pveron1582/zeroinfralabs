...
function AnimatedMission({ mission, fullText, titleText, done, active, index }: { mission: Mission, fullText: string, titleText: string, done: boolean, active: boolean, index: number }) {
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-0.5">
            ...
            <h4 className={`text-xs font-semibold leading-tight ${done ? 'text-emerald-400' : 'text-amber-300'}`}>{titleText}</h4>
          </div>
      ...
      {showHelp ? (
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 overflow-x-hidden" style={{ scrollbarWidth: 'thin', scrollbarColor: '#374151 transparent' }}>
          {visibleMissions.map((m, idx) => {
            const missionTitle = language === 'es' && m.titleEs ? m.titleEs : m.title;
            const missionDesc = language === 'es' && m.descriptionEs ? m.descriptionEs : m.description;
            return (
              <AnimatedMission 
                key={m.id} 
                mission={m} 
                fullText={resolve(missionDesc, m.targetMachineId)} 
